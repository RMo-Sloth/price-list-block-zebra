...
function UpdatePriceRecord( props ) {
    ...
    function process_changes(){
        if( price === props.record.price ) return;

        const formatted_price = Number( price ).toFixed(2);
        props.onUpdate( { ...props.record, price: formatted_price } );
        set_price( Number( price ).toFixed(2) );
    }
