...
export class PriceRecordCollectionFunctions {
	...
	static update(records: PriceRecordCollection, updated_record: PriceRecord): PriceRecordCollection {
		const old_record = PriceRecordCollectionFunctions.find_by_id(records, updated_record);
		const new_records = PriceRecordCollectionFunctions.replace( records, old_record, updated_record );
	...
	private static find_by_id( records: PriceRecordCollection, record: PriceRecord ): PriceRecord {
		return records.find( ({index}) => index === record.index );
	}
	...
	private static latestId(records: PriceRecordCollection): number {
		return records.reduce((prev, current) => Math.max(prev, current.index), 0);
	}

	/** Replaces records that exists. If one record matches it will be replaced by the other one. When both records match they will swap places. When no records match nothing will happen */
